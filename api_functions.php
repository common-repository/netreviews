...
function ntav_updateFlag(array $ids)
{
    ...
    $wcVersion = ntav_getConfig('WCVERSION');
    if (version_compare($wcVersion, '8.2.0', '>=')) {
        $query_meta = "UPDATE " . $table_prefix . "wc_orders_meta SET meta_value = '1' WHERE meta_key = 'av_flag' AND order_id IN (" . $where . ")";
...
function ntav_setFlagOrders($message)
{
    ...
    if ($message['datePeriod'] == 'periodOrders') {
        if (version_compare(ntav_getConfig('WCVERSION'), '8.2.0', '>=')) {


            $args = array(
                'return' => 'ids',
                'date_created' => $message['startDate'] . '...' . $message['endDate'],
                'type' => 'shop_order',
                'meta_key' => 'av_flag',
                'meta_value' => (''.$message['setFlag'] === '1' ? '0' : '1'),
                'meta_type' => 'numeric',
                'meta_query' => [
                    [
                        'key' => 'av_flag',
            ...
            $idsImploded = implode(',', $ordersToFlag);
            if (strlen($idsImploded)) {
                $query_meta = "UPDATE " . $table_prefix . "wc_orders_meta SET meta_value = " . $message['setFlag']
                    . " WHERE meta_key = 'av_flag' AND meta_value != " . $message['setFlag'] . " AND order_id IN (" . $idsImploded . ")";
                $results82 = $wpdb->query($query_meta);
            } else {
                $results82 = 0;
            }
        }
        ...
        if (version_compare(ntav_getConfig('WCVERSION'), '8.2.0', '>=')) {
            $query_meta = "UPDATE " . $table_prefix . "wc_orders_meta SET meta_value = " . $message['setFlag'] . " WHERE meta_key = 'av_flag' AND meta_value != " . $message['setFlag'];
            $results82 = $wpdb->query($query_meta);
        }
    }
...
/**
 * Fonction qui reviens avec des stats sur un ref produit
 * @param array $message contient les valeurs de configuration a modifie
 * @return array renvoi dans un tableau qui contient les stats sur un ref produit dans nos tables
 */
function ntav_getProductReviewsStats($message)
{
    global $wpdb;
    ...
    $refProduct = $message["refProduct"];

    $WpmlEnable = ntav_getWpmlEnable();

    if ($WpmlEnable == 'yes') {
        $response["nb reviews in av_products_average table"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT nb_reviews
                FROM " . $wpdb->prefix . "netreviews_products_average WHERE ref_product = %d AND lang = %s",
                $message["refProduct"],
                $message['lang_code']
            )
        );
        $response["average in av_products_average table"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT rate
                FROM " . $wpdb->prefix . "netreviews_products_average WHERE ref_product = %d AND lang = %s",
                $message["refProduct"],
        ...
        $response["nb reviews in av_products_reviews table"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)
                FROM " . $wpdb->prefix . "netreviews_products_reviews WHERE ref_product = %d AND lang = %s",
                $message["refProduct"],
                $message['lang_code']
            )
        );
        ...
        $response["nb reviews in av_products_average table"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT nb_reviews
                FROM " . $wpdb->prefix . "netreviews_products_average WHERE ref_product = %d",
                $message["refProduct"]
            )
        );
        $response["average in av_products_average table"] = $wpdb->get_var(
        ...
        $response["nb reviews in av_products_reviews table"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)
                FROM " . $wpdb->prefix . "netreviews_products_reviews WHERE ref_product = %d",
                $message["refProduct"]
            )
        );
