...
foreach ($output as $k_review => $review) {
    ...
    if (!empty($discussions)) {
        ...
        foreach (array_reverse($discussions) as $k_Discussion => $discussion) {
            ...
            if(strtotime($discussion->horodate) !== false){
                $discussion->horodate = date('d/m/Y', strtotime($discussion->horodate));
            } else {
                $discussion->horodate = date('d/m/Y', $discussion->horodate);
            }
            ...
            $html .= '<span class="netreviews_answer_title">' . __(
                'Reply from ',
                'av'
            ) . $discussion->origine . '<small> ' . __('on', 'av') . " " .$discussion->horodate. '</small></span>';
            $html .= '</br>';
