...
function appelApi()
{
    ...
    switch ($POST_DATA['query']) {
        ...
        case 'getProductReviewsStats':
            $reply = ntav_getProductReviewsStats($message);
            break;
        default:
            break;
    }
...
{
    $base64encodeSvgIcon = base64_encode(file_get_contents(dirname(__file__) . '/includes/images/icon-blue.svg'));
    ...
    add_menu_page(
        ...
        'av',
        'ntav_av_backoffice',
        "data:image/svg+xml;base64,$base64encodeSvgIcon",
        50
    );
...
function ntav_av_new_order($id)
{
    ...
    if (!is_numeric($flag_mark) && $flag_mark != 1) { //if order is created already in postmeta, avoid duplication
        ...
        if(version_compare($wcVersion, '8.2.0', '>=')) {
            $wpdb->query("INSERT INTO " . $wpdb->prefix . "wc_orders_meta (order_id, meta_key, meta_value) VALUES (" . $id . ", 'av_flag', '0')");
        }
    }
}
